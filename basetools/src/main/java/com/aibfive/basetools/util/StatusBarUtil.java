...
public class StatusBarUtil {
    ...
    /**
     *
     * 设置View的父类一个statusbar 高度的Toppadding
     *
     * @param view
     */
    public static void setParentStatusBarPadding(Context context, View view) {
        int statusBarHeight = 0;//小于4.4的 不能使用沉浸式 所以这里默认设置0
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            statusBarHeight = getStatusBarHeight(context);
        }
        ViewGroup group = (ViewGroup)view.getParent();
        int top = group.getPaddingTop() + statusBarHeight;
        int bottom = group.getPaddingBottom();
        int left = group.getPaddingLeft();
        int right = group.getPaddingRight();
        group.setPadding(left, top, right, bottom);
        /*if(parent instanceof LinearLayout){
            LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) ((LinearLayout)parent).getLayoutParams();
            params.topMargin = statusBarHeight + params.topMargin;
            view.setLayoutParams(params);
        }else if(parent instanceof FrameLayout){
            FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) ((FrameLayout)parent).getLayoutParams();
            params.topMargin = statusBarHeight + params.topMargin;
            view.setLayoutParams(params);
        }else if(parent instanceof RelativeLayout){
            RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) ((RelativeLayout)parent).getLayoutParams();
            params.topMargin = statusBarHeight + params.topMargin;
            view.setLayoutParams(params);
        }else if(parent instanceof ConstraintLayout){
            ConstraintLayout.LayoutParams params = (ConstraintLayout.LayoutParams) ((ConstraintLayout)parent).getLayoutParams();
            params.topMargin = statusBarHeight + params.topMargin;
            view.setLayoutParams(params);
        }*/
    }
